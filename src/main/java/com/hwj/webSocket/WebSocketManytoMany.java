...
public class WebSocketManytoMany {
     ...
     public void onMessage(String message, Session session) {
         ...
         for(WebSocketManytoMany item: webSocketSet){
             try {
                 ...
                 String role = (String) json.get("role");  //需要发送的信息
                 String room = (String) json.get("room");  //需要发送的信息
                 
                 item.sendMessage(role, room, string);
             } catch (IOException e) {
                 e.printStackTrace();
                 continue;
             }
    ...
    public void sendMessage(String role, String room, String string) throws IOException{
          this.session.getBasicRemote().sendText(role+"："+string);
     }
