...
/*--------------------------- 系统异常信息查看 -------------------------------*/
function exceptionController($scope,$http,$window,$rootScope,$filter) {
    ...
    $scope.pagenation = function (page,pageSize) {
        ...
        $http.post('admin/queryExpectionMsg?firstDate1='+firstDate+'&lastDate1='+lastDate+'&page='+page+'&size='+pageSize)
            .then(function (response) {
                $scope.totalNum = response.data.data.totalElements;//数据总数
                $scope.pages = response.data.data.totalPages;//页数
                $scope.currPage = response.data.data.number;//当前页
    ...
    /*模态框信息*/
    $scope.detailInfo = function (ip,params,errorMsg,url,time) {

        $scope.excMsgIP = ip;
        $scope.excMsgParams = params;
        $scope.excMsgErrorMsg = errorMsg;
        $scope.excMsgUrl = url;
        $scope.excMsgTime = time;
